...
-- 접속창 -> 계정열고 -> 테이블 클릭 -> 위쪽 새로고침 클릭 -> MEMBER 테이블 보임
...
-- 주민등록 번호의 데이터 타입을 NUMBER로 지정할 경우 문제점
...
CREATE TABLE USER_USED_NN(
    USER_NO NUMBER NOT NULL,  --사용자 번호(학번 같은 느낌, 무조건 다들 있어야 함 == NULL X)
                    --> 컬럼레벨 제약조건 설정
    USER_ID VARCHAR2(20) ,
...
-- ORA-01400: NULL을 ("BDH"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(BDH.USER_ID_U)에 위배됩니다
                --> UNIQUE
...
-- ORA-00001: 무결성 제약 조건(BDH.USER_ID_NAME_U)에 위배됩니다
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(BDH.USER_NO_PK)에 위배됩니다 == UNIQUE 오류와 같음

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("BDH"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다 == NOT NULL 오류와 같음
...
-- ORA-00001: 무결성 제약 조건(BDH.PK_USERNO_USERID)에 위배됩니다
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER -- CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE(GRADE_CODE) -- 컬럼 레벨 FK 설정
  ,
...
-- ORA-02291: 무결성 제약조건(BDH.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
            --> 참조 무결성 위배
...
WHERE GRADE_CODE = 10;
-- ORA-02292: 무결성 제약조건(BDH.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
-- ORA-02290: 체크 제약조건(BDH.GENDER_CHECK)이 위배되었습니다
