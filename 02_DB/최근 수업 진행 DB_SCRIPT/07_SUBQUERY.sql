...
-- 1) 부서별 급여 합 중 가장 큰값 조회 (부서별 급여합 1등)
...
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME,
    CASE WHEN EMP_ID IN (200,216) THEN '사수'
    ELSE '---'
    END 구분
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
SELECT EMP_ID, EMP_NAME, MANAGER_ID, 
    NVL( (SELECT EMP_NAME FROM EMPLOYEE E2 WHERE E2.EMP_ID = E1.MANAGER_ID) , '없음')  관리자명
    -- 메인 쿼리에서 MANAGER_ID를 얻어와  서브쿼리에서 같은 EMP_ID를 가진 사원의 이름을 조회
FROM EMPLOYEE E1;
