...
SELECT EMP_ID, EMP_NAME, JOB_NAME, 
    ...
    FROM EMPLOYEE
    JOIN JOB USING(JOB_CODE)
WHERE (JOB_CODE, EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO, 1, 2), 'RR'))) IN
        -- 직급별 나이가 가장 어린 직원 조회
        (SELECT JOB_CODE, MIN(EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO, 1, 2), 'RR')))
        FROM EMPLOYEE
        GROUP BY JOB_CODE)
ORDER BY 나이 DESC;
